...
"""
    from_continuous(x; weighted = true)
...
The `weighted` option applies only when the datavalues of the palette are of type `Bin`.
In this case, if `weighted = true`, the continuous colormap is sampled at the relative
midpoints of the bins, which means that neighboring bins that are smaller have more similar
colors because their midpoints are closer together. If `weighted = false`, the colormap
is sampled evenly.
...
```
"""
from_continuous(x; weighted = true) = FromContinuous(x, weighted)